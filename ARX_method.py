            ...
            if (na ==0 and nb ==0):
                ...
                for i in range(num_input):
                    for j in range(num_output):
                        ...
                        delay_temp = arx_return[0]
                        delay.append(delay_temp)
                        temp = AIC_cal(output_data.T,input_data.T,theta[num],delay[num],na,nb,data_size)
                        AIC_temp[num] = temp[0]
...
# tuning parameters
maxdelay = 100 #range of delay to search for the best result
na = 2  #Order of polynomial A(q) for ARX model
nb = 1 #Order of polynomial B(q) for ARX model
