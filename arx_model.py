...
class ARX(tsbase.TimeSeriesModel):
    __doc__ = tsbase._tsa_doc % {"model" : "Autoregressive ARX(p) model",
    ...
    def __init__(self, endog, dates=None, freq=None, missing='none'):
        super(ARX, self).__init__(endog, None, dates, freq, missing=missing)
    ...
    def _get_predict_start(self, start, dynamic):
        ...
        if start is None:
            ...
            start = super(ARX, self)._get_predict_start(start)
    ...
    def _loglike_css(self, params):
        """
        Loglikelihood of ARX(p) process using conditional sum of squares
        """
        nobs = self.nobs
        Y = self.Y
        X = self.X
    ...
    def _loglike_mle(self, params):
        """
        Loglikelihood of ARX(p) process using exact maximum likelihood
        """
        nobs = self.nobs
        X = self.X
        endog = self.endog
    ...
    def loglike(self, params):
        """
        The loglikelihood of an ARX(p) process

        Parameters
        ----------
        params : array
            The fitted parameters of the ARX model
        ...
        mean of the ARX process and :math:`\\sigma^{2}V_{p}` is the (`p` x `p`)
    ...
    def select_order(self, maxlag, ic, trend='c', method='mle'):
        ...
        ----------
        maxlag : int
            The highest lag length tried. See `ARX.fit`.
        ...
        # make Y and X with same nobs to compare ICs, not adding delay
        Y = endog[maxlag:]
        ...
        if ic != 't-stat':
            for lag in range(k, maxlag+1):
                ...
                fit = ARX(endog_tmp).fit(maxlag=lag, method=method,
    ...
    def fit(self, maxlag=None, method='cmle', ic=None, trend='c',
        ...
        """
        Fit the unconditional maximum likelihood of an ARX(p) process.
        ...
        if method == "cmle":     # do OLS
            ...
            mlefit = super(ARX, self).fit(start_params=start_params,
...
class ARResults(tsbase.TimeSeriesModelResults):
    """
    Class to hold results from fitting an ARX model.

    Parameters
    ----------
    model : ARX Model instance
        Reference to the model that is fit.
    params : array
        The fitted parameters from the ARX Model.
    ...
    **Attributes**
    ...
    fittedvalues : array
        The in-sample predicted values of the fitted ARX model. The `k_ar`
    ...
    llf : float
        The loglikelihood of the model evaluated at `params`. See `ARX.loglike`
    model : ARX model instance
        A reference to the fitted ARX model.
    nobs : float
    ...
    resid : array
        ...
        Filter.
    roots : array
        The roots of the ARX process are the solution to
    ...
    preddoc = ARX.predict.__doc__.split('\n')
...
# Why does R demean the data by defaut?
    ar_ols = ARX(sunspots.endog)
    res_ols = ar_ols.fit(maxlag=9)
    ar_mle = ARX(sunspots.endog)
...
#    ar_yw = ARX(sunspots.endog)
#    res_yw = ar_yw.fit(maxlag=4, method="yw")

#    # Timings versus talkbox
#    from timeit import default_timer as timer
#    print "Time ARX fit vs. talkbox"
#    # generate a long series of ARX(2) data
#
#    nobs = 1000000
#    y = np.empty(nobs)
#    y[0:2] = 0
...
#    t_end = timer()
#    print str(t_end - t) + " seconds for sm.ARX with yule-walker, 2 lags"
#    try:
