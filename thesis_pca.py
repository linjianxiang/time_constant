...
def DPCA_cal(x,h,UCL=0):      
    ...
    dfn = p_component_num
    dfd = N - dfn
    rv = f(p_component_num,N-p_component_num)
    F = rv.cdf(p_component_threshold)
    ...
    C_a = stats.norm.cdf(p_component_threshold) # should be wrong ca is standard normal variable corresponding to the upper 1-p_component_threshold percentile
   


    if not UCL:
        UCL_T2 = p_component_num*(N-1)*(N+1)/(N**2 -N*p_component_num)*F
        T2_fault_indices = detect_peaks(T2_hotelling, mph=UCL_T2,mpd=50,edge = 'rising', show=True,UCL = UCL_T2)
        ...
        SPE_fault_indices = detect_peaks(SPE, mph=UCL_SPE,UCL=UCL_SPE,mpd=50,edge = 'rising',NAME = 'spe no fault')
        print("SPE UCL is",UCL_SPE,"SPE detected", len(SPE_fault_indices),'faults')
    else:
        T2_fault_indices = detect_peaks(T2_hotelling, mph=UCL[0],mpd=50,edge = 'rising', show=True,UCL = UCL[0],NAME = 'with fault')
        print("T2_hotelling UCL for non_fault is",UCL[0],"T2_hotelling detected", len(T2_fault_indices),'faults')
        SPE_fault_indices = detect_peaks(SPE, mph=UCL[1], show=True,UCL=UCL[1],mpd=50,edge = 'rising',NAME = 'spe with fault')
...
## ARX model parameter&costs calcuation, for finding best parameter
...
## AIC test for finding the best order(na,nb)
## exhausted searching for the order, slow
...
## plot reconstructed model, parameter astemated by ARX##
...
##RR-13 method for dpca lag calculation -- very slow ##
def EstimateLag(Xtr):
    lag=0
...
def h_estimate(x,num_output,na_max,nb_max,maxdelay):
    ### start h estimate####
    # initial param
    num_data = x.shape[1]
...
### training start ########
## load data
[x,num_output] =  CSTR_data_load() # cstr data
#[x,num_output] =  TE_train_data_load() #TE data

## h estimate
na_max = 10
nb_max = 5
maxdelay = 20 #range of delay to search for the best result
[delay,order] = h_estimate(x,num_output,na_max,nb_max,maxdelay) #estimated order

## UCL calculation
UCL_nofault = DPCA_cal(x,order)
# h_corr_method = EstimateLag(x) #13 estimation by the RR-13
# print('h correlation method', h_corr_method)
###training done########

### testing ##
plot_size = x.shape[0]
TE_test_data_all(order,plot_size)
### testing done###
